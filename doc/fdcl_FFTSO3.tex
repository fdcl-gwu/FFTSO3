...
A function $f\in\mathcal{L}^2(\SO)$ is called \textit{band-limited} with the band $B$ if $f^l=0$ for any $l\geq B$ in \refeqn{f_IFT}, or equivalently
\begin{align*}
f(R(\alpha,\beta,\gamma)) &= \sum_{l=0}^{B-1} \sum_{m,n=-l}^l (2l+1)\bar f^l_{m,n} D^l_{m,n}(\alpha,\beta,\gamma).
\end{align*}
...
\alpha_j=\gamma_j= \frac{\pi j}{2B},\quad \beta_k = \frac{\pi (2k+1)}{4B}.
\end{equation}
...
&=\sum_{j_1,k,j_2=0}^{2B-1} w_k (D^l_{m,n}(R(\alpha_{j_1},\beta_k,\gamma_{j_2})))^*\\
...
\[
s(R)=1+\sum_{l=2B}^\infty \sum_{m,n=-l}^l \bar s^l_{m,n} D^l_{m,n}(R).
\]
Then,
\begin{align*}
f_s(R)&=f(R)+f(R)\sum_{l=2B}^\infty \sum_{m,n=-l}^l \bar s^l_{m,n} D^l_{m,n}(R).
\end{align*}
...
&=\sum_{j_1=0}^{2B-1} 
e^{im\alpha_{j_1}}\sum_{j_2=0}^{2B-1} e^{in\gamma_{j_2}} \sum_{k}^{2l_{\max}-1}w_k f(\alpha_{j_1},\beta_k,\gamma_{j_2})d^l_{m,n}(\beta_k) \\
\end{align*}
...
F^{\gamma}_{l,m,n}(j_1)&=\sum_{j_2=0}^{2B-1}e^{in\gamma_{j_2}}F^{\beta}_{l,m,n}(j_1,j_2),\\
f^l_{m,n} & = \sum_{j_1=0}^{2B-1} e^{im\alpha_{j_1}}F^{\gamma}_{l,m,n}(j_1).
\end{align*}
